...
function genTower(floor: number) {
...
export default function CsbLevelTower() {
  ...
  /* ----------------------------- floor build ---------------------------- */
  const buildTier = useCallback((f: number, keepHp = true) => {
    const s = g.current;
    const { grid, rooms } = genTower(f);
  ...
  /** Bank progress mid-run (called on each ascend) so nothing is ever lost */
  ...
  const draw = (ctx: CanvasRenderingContext2D, cv: HTMLCanvasElement) => {
    ...
    ctx.fillText(`${b.emoji} ${b.name} · Tier ${s.floor}`, mmx, mmy - 6);
  };
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[...heldBulls].sort((a, b) => (b.level || 1) - (a.level || 1)).map((b, i) => (
                  <Card key={b.nft_id} onClick={() => startRun(b)}
                    ...
                    <Button size="sm" className="w-full mt-2"><Swords className="w-3 h-3 mr-1" /> Ascend</Button>
                  </Card>
                ))}
              </div>
            </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950/40 to-slate-950 text-foreground p-4 flex items-center justify-center">
        <Card className="p-8 max-w-md w-full text-center bg-slate-900/80 border-slate-700 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div className="p-3 rounded bg-black/30"><div className="font-bold text-lg">{runSummary.floors}</div>Tiers</div>
          </div>
  ...
  return (
    <div className="fixed inset-0 bg-black text-foreground select-none">
      ...
      <div className="absolute bottom-4 right-4 flex flex-col items-end gap-2">
        ...
        <Button size="sm" variant="secondary" onClick={tryInteract}>ASCEND</Button>
        {nearExit && (
          <div className="text-[10px] text-cyan-200/80 text-right max-w-[180px] pointer-events-none">
            {g.current.bossAlive ? "Slay the boss to open the portal" : "Reach 🪜 and press E / tap ASCEND"}
          </div>
        )}
