...
const SpaceRace = () => {
  ...
  const play = () => {
    ...
    const interval = setInterval(() => {
      setPositions(prev => prev.map(p => p + Math.floor(Math.random() * 10) + 5));
    }, 200);
    ...
    setTimeout(() => {
      ...
      setPositions(prev => {
        ...
        if (winner === choice) {
          ...
          toast.success(`🐂 ${ships[winner]} won! ${winAmount} credits!`);
        } else {
          toast.error(`${ships[winner]} won!`);
        }
        setPlaying(false);
        return final;
      });
    ...
    <div className="min-h-screen bull-pattern p-4">
      ...
      <Card className="max-w-4xl mx-auto p-6 bg-card/95 backdrop-blur">
        <div className="text-center mb-6">
          <h1 className="text-4xl font-bold gradient-gold bg-clip-text text-transparent mb-2">Space Race 🐂</h1>
          <p className="text-muted-foreground">Pick your ship to win!</p>
